...
class LowerLimbKinematicsDH:
    ...
    def update(self, timestamp: float,
        ...
        feat = {
            ...
            # 关节角度
            "left_hip": left_hip_rad,
            "right_hip": right_hip_rad,
            "left_knee": left_knee_rad,
            "right_knee": right_knee_rad,

            # ===== 新增：编码器速度 =====
            "left_knee_vel": left_knee_encoder.velocity,
            "right_knee_vel": right_knee_encoder.velocity,

            # ===== 新增：编码器反馈 / 前馈力矩 =====
            "left_knee_tff": left_knee_encoder.t_ff,
            "right_knee_tff": right_knee_encoder.t_ff,
        ...
        def fill_imu(prefix: str, imu: IMUData):
            feat[f'{prefix}_acc_x'] = imu.accelerometer[0]
            feat[f'{prefix}_acc_y'] = imu.accelerometer[1]
            feat[f'{prefix}_acc_z'] = imu.accelerometer[2]
            feat[f'{prefix}_gyro_x'] = imu.gyroscope[0]
            feat[f'{prefix}_gyro_y'] = imu.gyroscope[1]
            feat[f'{prefix}_gyro_z'] = imu.gyroscope[2]
            feat[f'{prefix}_quat_w'] = imu.quaternion[0]
            feat[f'{prefix}_quat_x'] = imu.quaternion[1]
            feat[f'{prefix}_quat_y'] = imu.quaternion[2]
            feat[f'{prefix}_quat_z'] = imu.quaternion[3]

        fill_imu('pelvis', pelvis_imu)
        fill_imu('left_thigh', left_thigh_imu)
        fill_imu('right_thigh', right_thigh_imu)

        # DH 参数 - 与 core 一致的键名格式
        for i, dh in enumerate(left_dh):
            feat["dh"][f"L{i}_theta"] = dh.theta
        for i, dh in enumerate(right_dh):
            feat["dh"][f"R{i}_theta"] = dh.theta

        # FK 位置 - 与 core 一致的键名格式
        for j, p in enumerate(left_positions):
        ...
        self.history["left_knee_vel"].append(left_knee_encoder.velocity)
        self.history["right_knee_vel"].append(right_knee_encoder.velocity)

        self.history["left_knee_tff"].append(left_knee_encoder.t_ff)
        self.history["right_knee_tff"].append(right_knee_encoder.t_ff)


        return feat

    # -------------------- 导出 CSV（展平嵌套字典）--------------------
    def export_features_to_csv(self, filepath: str = "all_features.csv"):
        """
        将 feature_history 导出为 CSV，展平嵌套的 dh 和 fk 字典
        列顺序与 feat 结构完全一致
        """
        flat_records = []
        
        # 定义固定的列顺序，与 feat 结构一致
        base_cols = [
            ...
            "left_knee_tff", "right_knee_tff"
        ]

        imu_cols = []
        for prefix in ["pelvis", "left_thigh", "right_thigh"]:
            imu_cols.extend([
                f"{prefix}_acc_x", f"{prefix}_acc_y", f"{prefix}_acc_z",
                f"{prefix}_gyro_x", f"{prefix}_gyro_y", f"{prefix}_gyro_z",
                f"{prefix}_quat_w", f"{prefix}_quat_x", f"{prefix}_quat_y", f"{prefix}_quat_z"
            ])
        dh_cols = [f"L{i}_theta" for i in range(5)] + [f"R{i}_theta" for i in range(5)]
        fk_cols = []
        for j in range(6):
            fk_cols.extend([f"L_fk_{j}_x", f"L_fk_{j}_y", f"L_fk_{j}_z"])
        for j in range(6):
            fk_cols.extend([f"R_fk_{j}_x", f"R_fk_{j}_y", f"R_fk_{j}_z"])
        
        all_cols = base_cols + imu_cols + dh_cols + fk_cols
        
        for feat in self.feature_history:
            flat = {}
            # 基础列
            flat["timestamp"] = feat.get("timestamp")
            flat["system_time"] = feat.get("system_time")
            flat["left_hip"] = feat["left_hip"]
            flat["right_hip"] = feat["right_hip"]
            flat["left_knee_angle"] = feat["left_knee"]
            flat["right_knee_angle"] = feat["right_knee"]

            flat["left_knee_vel"] = feat.get("left_knee_vel")
            flat["right_knee_vel"] = feat.get("right_knee_vel")

            flat["left_knee_tff"] = feat.get("left_knee_tff")
            flat["right_knee_tff"] = feat.get("right_knee_tff")

            # IMU 列
            for col in imu_cols:
                flat[col] = feat.get(col)
            # DH 列
            for col in dh_cols:
                flat[col] = feat.get("dh", {}).get(col)
            # FK 列
            for col in fk_cols:
                flat[col] = feat.get("fk", {}).get(col)
            
            flat_records.append(flat)
        
        df = pd.DataFrame(flat_records, columns=all_cols)
        df.to_csv(filepath, index=False, encoding="utf-8")
        print(f"已导出 {filepath}（键值与 core.py 一致）。")
        return df
...
if __name__ == "__main__":
    ...
    for i in range(n):
        try:
            ...
            # 获取 system_time
            sys_time = imu0.iloc[i].get('System_Time', None)
        except Exception as e:
            print("行解析错误, 跳过帧:", i, e)
            continue
    ...
    # 使用新的导出方法（键值与 core.py 一致）
    model.export_features_to_csv("all_features.csv")
